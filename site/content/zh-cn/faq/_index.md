...
先跑 [`examples/tutorial`](/zh-cn/tutorials/)，再对照 `pkg/session` / `pkg/goal` 精读，然后深入 `pkg/agent` / `pkg/tools` / `pkg/llm`。
...
见[教程：事件溯源](/zh-cn/tutorials/event-sourcing/)。

## Q10. 什么是 fold 投影？

见[教程：fold 投影](/zh-cn/tutorials/fold-projection/)。

...
| [docs/TEST_CASES.md](./docs/TEST_CASES.md) | **🧪 测试工程师 / 验收**（全量测试用例设计表）| Markdown 表格（98 项功能点 → 328 条用例）| ✅ **任务状态变化时同步用例状态列** |
| [docs/TEST_CASES_REPORT.html](./docs/TEST_CASES_REPORT.html) | **📊 交互式浏览**（浏览器打开，可搜索/筛选/排序）| 自包含 HTML（零依赖）| 由 TEST_CASES.md 派生，随其重建 |
...
| 🔴 MUST（N01-N07 缓存） | 7 | 0 | 0 | 7 | 0 | 0 | 0 | **100.00%** |
| 🟡 SHOULD（S01-S16） | 16 | 1 | 0 | 15 | 0 | 0 | 0 | **93.75%** |
| 🟡 SHOULD（N08-N09 缓存） | 2 | 0 | 0 | 2 | 0 | 0 | 0 | **100.00%** |
| 🔴 + 🟡 合计（非跳过） | **73** | **1** | **0** | **72** | **0** | **0** | **0** | **98.63%** |
